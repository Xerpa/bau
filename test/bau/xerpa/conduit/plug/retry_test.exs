defmodule Bau.Xerpa.Conduit.Plug.RetryTest do
  use ExUnit.Case
  import Conduit.Message
  import ExUnit.CaptureLog

  describe "when a message is rejected" do
    defmodule RejectedRetry do
      use Conduit.Subscriber
      plug(Bau.Xerpa.Conduit.Plug.Retry, attempts: 2, delay: 2)
    ...
    test "does not retry and returns the rejected message" do
      assert %Conduit.Message{status: :reject} = RejectedRetry.run(%Conduit.Message{})

      assert_received({:process, first_message})
      refute_received({:process, _second_message})
